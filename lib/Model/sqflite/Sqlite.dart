import 'package:dd_master_certo/Domain/Usuario.dart';
...
class Sqflite {
  ...
  static Future<Database?> get() async {
    if (_db == null) {
      ...
      _db?.rawInsert(
          "INSERT INTO classe (id , nome, habilidade, ataque, defesa, vida, agilidade, mana) VALUES (?,?,?,?,?,?,?,?)",
          [
            1,
            "Mago",
            "Conjurador de magias excepcionais",
            1,
            30.0,
            5.0,
            10.0,
  ...
  static Future<Future<int>?> salvarHabilidade(
      String nome, String descricao, String tipo, double porcentagem,
      [int? id]) async {
    get();
  ...
  static Future<Usuario?> getUsuarioByEmail(String email) async {
    List<Map<String, Object?>>? usuarioResult = await _db
        ?.rawQuery('SELECT * FROM usuario u WHERE u.email = ?', [email]);
